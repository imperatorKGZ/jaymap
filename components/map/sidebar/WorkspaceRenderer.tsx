...
function WorkspaceFallback() {
  ...
  return (
    <div
      ...
      aria-busy="true"
      aria-label={t("sidebar.loadingAria")}
    >
      {[0, 1, 2].map((i) => (
        <div
          key={i}
...
function WorkspaceRendererBase({
  ...
  const defaults =
    WORKSPACE_DEFAULTS[workspaceKey] ?? {};

  const values = getSectionFilters(
    sectionId,
    defaults
  );
...
export const WorkspaceRenderer = memo(
