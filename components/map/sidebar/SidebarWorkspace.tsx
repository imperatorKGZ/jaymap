...
interface SidebarWorkspaceProps {
  ...
  state: Pick<SidebarState, "openMain" | "closeMain" | "closeSecondary" | "getSectionFilters" | "setFilter">;
  onApply?: () => void;
}
...
function SidebarWorkspaceBase({
  ...
  if (secondarySection) {
    return (
      <div className="flex h-full flex-col">
        <SidebarHeader
          title={t(secondarySection.titleKey)}
          backLabel={activeMainSection ? t(activeMainSection.titleKey) : undefined}
          onBack={state.closeSecondary}
        />
        <div className="sb-scroll flex-1 overflow-y-auto">
          <WorkspaceRenderer
            workspaceKey={secondarySection.workspace}
            sectionId={secondarySection.id}
            getSectionFilters={state.getSectionFilters}
  ...
  if (activeMainSection) {
    return (
      <div className="flex h-full flex-col">
        <SidebarHeader title={t(activeMainSection.titleKey)} onBack={state.closeMain} />
        <div className="sb-scroll flex-1 overflow-y-auto">
          <WorkspaceRenderer
            workspaceKey={activeMainSection.workspace}
            sectionId={activeMainSection.id}
            getSectionFilters={state.getSectionFilters}
            setFilter={state.setFilter}
            onSubmit={onApply}
          />
        ...
        <div className="flex flex-col gap-1">
          {mainSections.map((section) => (
            <SidebarItem
              ...
              onClick={() => state.openMain(section.id)}
            />
          ))}
        </div>
      </div>
...
export const SidebarWorkspace = memo(SidebarWorkspaceBase);
